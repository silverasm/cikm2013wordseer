...
We will use the first case study:  US-China relations, to illustrate the text-sliding capabilities of our tool.
...
=======
>>>>>>> 7184ce43cc80defad271dc7614cc79c802d20b6a
...
When users open up the tool for the first time, they see something like Figure \ref{fig:intro01}. This may seem like a lot of information, but Figure \ref{fig:intro02} breaks it down into simple components. It's a single \strong{view} with a a \strong{Word Tree} visualization of the most frequent word in the \strong{slice} ``the entire collection''. 
...
\subsection{Views}
Views  are window-like panels, and they contain the following components, as deconstructed in Figure \ref{fig:intro02}:
\begin{itemize}
...
For his project, Fan was interested characterizing the way China was talked about in the three decades from 1980 to 2009. To do this, he assembled three slices by starting with a \emph{search} for ``China'' (Figure \ref{fig:intro03}) and and then filtering it by decade (Figure \ref{fig:intro04a}). 
...
\begin{center}
%
        \subfigure[Fan used the numerical/date ranges to filter the slice ``all sentences containing \emph{China}" to sentences from the 90's. ]{%
            \label{fig:intro04a}
...
\end{itemize}
By traversing these indexes both ways, we can quickly compute the associations for a slice. From a slice, we can query for all the words, phrases, or grammatical relations in the sentences in that slice, and from there, to all the other sentences that contain each particular item.

In fact, we present the most common nouns, verbs, and adjectives, as well as the most common phrases the the user automatically. As shown in Figures \ref{fig:intro01} and \ref{fig:intro02}, 
